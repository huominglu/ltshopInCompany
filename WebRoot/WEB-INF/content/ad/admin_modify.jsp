...
<script type="text/javascript">		
	$(document).ready(function(){
		var power = '${requestScope.admin.role.id}';
		$.ajax({
			type: 'post',
			url: 'getallrole',
			success: function(result){
				...
				$.each(jsonobj,function(i,value){
					if(power == value.id){
						var opo = "<option selected='selected' value="+value.id+">"+value.rolename+"</option>";
					}else{
		...
		$("#name").val("${requestScope.admin.name }");
		$("#password").val("${requestScope.admin.password }");
...
<div id="container">
    ...
    <div id="uboxstyle" align="center">	
        <form name="form1" action="modifyadmin" method="post">
            <table cellspacing="0" cellpadding="0">
             ...
             <tr>
                ...
                <td><input type="password" id="password" name="admin.password" onblur="checkpwd()" />
                	<input type="hidden" name="admin.id" value="${requestScope.admin.id }">
             ...
             <tr>
                <td>用户等级</td>
                <td>
                <select name="admin.role.id" id="language">
